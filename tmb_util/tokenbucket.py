...
def token_bucket(size_, refill_rate_):
    ...
    The two arguments to this function are the size of the token bucket
    (maximum number of tokens it can hold), and the rate at which the bucket
    refills with tokens (number of seconds between each additional earned
    token).
    ...
    - the amount of time that must be waited before doing another action; and
    - its new state '''
    ...
    def closure_token_bucket(state):
        ...
        if not state:
            state = {'tokens': size, 'last_action': 0}
        # By calling this function, we know we have performed an action and
        # must therefore spend a token
        state['tokens'] -= 1
        ...
        if state['tokens'] > 0:
            ...
            # if tokens=0,  1*refill_rate time must pass before positive tokens
            # if tokens=-1, 2*refill_rate time must pass before positive tokens
            # if tokens=-2, 3*refill_rate time must pass before positive tokens
            # ...
            # (1+(-1*tokens))*refill_rate
            return ((1 + (-1 * state['tokens'])) * refill_rate), state
